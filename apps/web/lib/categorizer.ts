...
function buildCategorizationPrompt(
  ...
  const categoriesList = allSlugs
    .map((slug) => `- ${slug}: ${categoryDescriptions[slug] ?? slug.replace(/-/g, ' ')}`)
    .join('\n')

  const tweetData = bookmarks.map((b) => {
    const entry: Record<string, unknown> = { tweetId: b.tweetId, text: b.text.slice(0, 400) }
...
- Use ALL signals: tweet text, hashtags, detected tools, semantic AI tags
...
export async function categorizeBatch(bookmarks: ItemForCategorization[]): Promise<CategorizationResult[]> {
  ...
  const text = await completePrompt(prompt, { maxTokens: 2048, timeoutMs: 60_000 })
...
export async function writeCategoryResults(results: CategorizationResult[]): Promise<void> {
  ...
  const [allCategories, rows] = await Promise.all([
    ...
    db.select({ id: items.id, tweetId: items.tweetId }).from(items).where(inArray(items.tweetId, tweetIds)),
  ])

  const categoryBySlug = new Map(allCategories.map((c) => [c.slug, c.id]))
  const itemByTweetId = new Map(rows.map((r) => [r.tweetId, r.id]))
  const now = new Date()
  const itemIdsToUpdate: string[] = []

  for (const result of results) {
    const itemId = itemByTweetId.get(result.tweetId)
  ...
  if (itemIdsToUpdate.length > 0) {
    await db.update(items).set({ categorizedAt: now }).where(inArray(items.id, itemIdsToUpdate))
  }
}
...
export function mapItemForCategorization(row: {
  ...
  if (row.entities) {
    try {
      const ent = JSON.parse(row.entities) as { hashtags?: string[]; tools?: string[] }
  ...
  return { tweetId: row.tweetId, text: row.text, tags, hashtags, tools }
}
