...
export const items = sqliteTable(
	"items",
	{
		...
		/** Soft-delete. Listed queries skip these; purge can drop them later. */
		archivedAt: integer("archived_at", { mode: "timestamp" }),
		/** Set when the library asks the extension to capture this article again. */
		hydrateRequestedAt: integer("hydrate_requested_at", { mode: "timestamp" }),
		/** X said this capture target is gone. Keep the saved row; skip auto-pending. */
		captureUnavailableAt: integer("capture_unavailable_at", { mode: "timestamp" }),
	},
	(table) => [
		...
		index("items_archived_at_idx").on(table.archivedAt),
